$(document).ready(function() {
	...
	$('#calc_period').change(function(){
		...
		if ( calc_period == 'mm' ) {
			...
			$('.calc__block--tohide3').css({display: "block"});
		...
		if ( calc_period == 'dd' ) {
			...
			$('.calc__block--tohide3').css({display: "none"});
	...
	function calculation() {
		...
		var calc_stavka = $('#calc_stavka').val();
		...
		if ( calc_period == 'dd' ) {
			...
			var calc_chart_period = calc_srok;
			calc_type = 3;
		}
		...
		if ( calc_type == 1 ) {
			...
			for (i = 0; i < calc_month; i++) {

				var calc_F = calc_credit * 
				(
					(calc_percent / 100 / calc_rate) + 
					( (calc_percent / 100 / calc_rate) / (((1 + (calc_percent / 100 / calc_rate)) ** (calc_month)) - 1) )
				);
		...
		if ( calc_type == 2 ) {
			...
			for (i = 0; i < calc_month; i++) {
				...
				if ( i == 0 ) {
					var calc_C = (calc_credit / 12 / calc_years);
					var calc_P = (calc_credit * calc_percent * calc_diff * 30.4166667 / 365 / 100)
					var calc_F = +calc_C + +calc_P;
					calc_sum_perc = calc_sum_perc + calc_P;
					calc_sum_platej = calc_sum_platej + calc_F;
				} else {
					var calc_C = (calc_credit / 12 / calc_years);
					var calc_P = ((+calc_credit - (calc_C * i)) * calc_diff * calc_percent * 30.4166667 / 365 / 100)
					var calc_F = +calc_C + +calc_P;
					calc_sum_perc = calc_sum_perc + calc_P;
					calc_sum_platej = calc_sum_platej + calc_F;
				}
				...
				$('#calc_result_1').text( "~" + (+(calc_sum_platej / calc_years).toFixed(0)).toLocaleString("ru") + " ₽" );
				$('#calc_result_2').text( (+calc_sum_platej.toFixed(0)).toLocaleString("ru") + " ₽" );
				$('#calc_result_3').text( (+calc_credit).toLocaleString("ru") + " ₽" );
				$('#calc_result_4').text( (+(+calc_sum_platej - +calc_credit).toFixed(0)).toLocaleString("ru") + " ₽" );
				$('#calc_result_5').text( calc_percent + "%" );
		...
		var bar_width = $('.chart__wrap').width() / calc_chart_period;
		...
		for (z = 0; z < calc_chart_period; z++) {
			...
			if ( calc_type == 3 ) {
				var bar_rate = calc_platej_full[calc_chart_period-1] / 100;
