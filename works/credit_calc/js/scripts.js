$(document).ready(function() {
	...
	function calculation() {
		...
		$('#calc_free').val( Math.ceil( calc_srok * 0.1 ) );
		...
		// console.log(calc_dateARR[calc_months-1]);

		if ( calc_type == 1 ) {

			for (i = 0; i < calc_months; i++) {

				var calc_F = calc_credit * 
				(
					(calc_percent / 100 / 12) + 
					( (calc_percent / 100 / 12) / (((1 + (calc_percent / 100 / 12)) ** (calc_months)) - 1) )
				);
				...
				if ( i == 0 ) {
					...
					var calc_P = (calc_credit) * calc_percent / 100 / 12;
					...
					var calc_P = (calc_dolg[i-1]) * calc_percent / 100 / 12;
					calc_sum_perc = calc_sum_perc + calc_P;
					calc_platej_perc.push(calc_P.toFixed(0));
			...
			for (i = 0; i < calc_months; i++) {
				...
				if ( i == 0 ) {
					...
					var calc_P = (calc_credit * calc_percent * 30.4166667 / 365 / 100)
					var calc_F = +calc_C + +calc_P;
					calc_sum_perc = calc_sum_perc + calc_P;
					calc_sum_platej = calc_sum_platej + calc_F;
				} else {
					var calc_C = (calc_credit / 12 / calc_years);
					var calc_P = ((+calc_credit - (calc_C * i)) * calc_percent * 30.4166667 / 365 / 100)
					var calc_F = +calc_C + +calc_P;
					calc_sum_perc = calc_sum_perc + calc_P;
					calc_sum_platej = calc_sum_platej + calc_F;
				}
				...
				$('#calc_result_1').text( "~" + (+(calc_sum_platej / calc_months).toFixed(0)).toLocaleString("ru") + " ₽" );
				$('#calc_result_2').text( (+calc_sum_platej.toFixed(0)).toLocaleString("ru") + " ₽" );
				$('#calc_result_3').text( (+calc_credit).toLocaleString("ru") + " ₽" );
				$('#calc_result_4').text( (+(+calc_sum_platej - +calc_credit).toFixed(0)).toLocaleString("ru") + " ₽" );
				$('#calc_result_5').text( calc_percent + "%" );
		...
		var bar_width = $('.chart__wrap').width() / calc_months;
		var bar_padding = bar_width * 0.1;


		$('.chart__wrap').empty();
		for (z = 0; z < calc_months; z++) {
